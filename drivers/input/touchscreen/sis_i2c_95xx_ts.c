...
#define ENABLE_SNR_1TP_MODE     0x01A1
#define DISABLE_SNR_MODE        0x01A0
...
#define SIG_MON_ADDR            0x56000000
		...
		char *buf)
{
	...
	ret = send_cmd85_change_mode(data, ENABLE_SNR_1TP_MODE);
	if (ret < 0)
		goto err_comm_fail;
	...
	ret = send_cmd85_change_mode(data, DISABLE_SNR_MODE);
	if (ret < 0)
		goto err_comm_fail;
...
static ssize_t sig_mon_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	uint8_t *sig_mon;
	uint8_t temp_buf[4];
	...
	/* read diff data */
	sig_mon = kmalloc(total_len + 1, GFP_KERNEL);
	if (!sig_mon) {
		ret = -ENOMEM;
		goto err_comm_fail;
	}

	ret = send_cmd86_read_debug(data, SIG_MON_ADDR, sig_mon, total_len);
	if (ret < 0)
		goto err_comm_fail;
	...
	for (i = 0; i < total_len; i += allign_rx, index += nrx)
		memcpy(&buf[index], &sig_mon[i], nrx);
...
static DEVICE_ATTR(sig_mon, S_IRUSR, sig_mon_show, NULL);
